...
class CommandDB:
    ...
    def __init__(self, db_location):
        ...
        cur.execute('''CREATE TABLE IF NOT EXISTS chat_commands (id INTEGER PRIMARY KEY AUTO_INCREMENT, command VARCHAR(50), command_type TEXT, permission TEXT, num_args INT, arguments TEXT, action TEXT, UNIQUE(command))''')
        cur.execute('''CREATE TABLE IF NOT EXISTS shoutouts (id INTEGER PRIMARY KEY AUTO_INCREMENT, command VARCHAR(50), so_user VARCHAR(50), chat_text TEXT, twitch_clip TEXT, UNIQUE(command, so_user))''')
    ...
    def init_db(self, command_file):
        ...
        with open(command_file) as f:
            ...
            for c in data["commands"]:
                sql_str = "INSERT INTO chat_commands(command,command_type,permission,num_args,arguments,action) VALUES(%s,%s,%s,%s,%s,%s)"
                print("Inserting", c["command"])
                cur.execute(sql_str, (c["command"], c["command_type"], c["permission"], int(
                    c["num_args"]), c["arguments"], c["action"]))
            self.connection_.commit()
    ...
    def add_command(self, command):
        self.open()
        cur = self.connection_.cursor()
        cur.execute('''INSERT IGNORE INTO chat_commands (command, command_type, permission, num_args, arguments, action) VALUES(%s,%s,%s,%s,%s,%s)''', command.get_fields())
        self.connection_.commit()
        self.close()
        return command
    ...
    def add_shoutout(self, shoutout):
        self.open()
        cur = self.connection_.cursor()
        command = Command((shoutout["command"], "CHAT", "MODERATOR", "0", "",
                          "chat(%s,%u,'"+" : ".join([shoutout["chat_text"], shoutout["twitch_clip"]])+"')"))
        cur.execute(
            '''INSERT IGNORE INTO shoutouts (command,so_user,chat_text,twitch_clip) VALUES(%s,%s,%s,%s)''', shoutout.get_fields())
        cur.execute('''INSERT IGNORE INTO chat_commands (command, command_type, permission, num_args, arguments, action) VALUES(%s,%s,%s,%s,%s,%s)''', command.get_fields())
        self.connection_.commit()
        self.close()
    ...
    def remove_command(self, command):
        self.open()
        cur = self.connection_.cursor()
        cur.execute('''DELETE FROM chat_commands WHERE command=%s''',
                    (command["command"],))
        self.connection_.commit()
        self.close()
    ...
    def get_command(self, command_name):
        ...
        cur = self.connection_.cursor()
        cur.execute(
            '''SELECT * FROM chat_commands WHERE command=%s''', (command_name,))
    ...
    def remove_shoutout(self, shoutout):
        ...
        cur.execute('''DELETE FROM chat_commands WHERE command=%s''',
                    (shoutout["command"],))
        cur.execute('''DELETE FROM shoutouts WHERE command=%s''',
                    (shoutout["command"],))
        self.connection_.commit()
        self.close()
    ...
    def get_shoutout(self, so_user):
        self.open()
        cur = self.connection_.cursor()
        cur.execute('''SELECT * FROM shoutouts WHERE so_user=%s''', (so_user,))
        so_obj = cur.fetchone()
        self.close()
        if so_obj:
...
class MusicDB:
    ...
    def __init__(self, db_location):
        ...
        cur.execute(
            '''CREATE TABLE IF NOT EXISTS playlist_requests (id INTEGER PRIMARY KEY, playlist_id INTEGER, num_requests INTEGER)''')
    ...
    def add_playlist(self, playlist):
        self.open()
        cur = self.connection_.cursor()
        cur.execute('''INSERT OR IGNORE INTO playlists(playlist_name,user_added,youtube_id) VALUES(?,?,?)''',
                    playlist.get_fields()[1:])
        rid = cur.lastrowid
        self.connection_.commit()
        self.close()
        return (True, rid)
    ...
    def add_track_to_playlist(self, track):
        self.open()
        cur = self.connection_.cursor()
        cur.execute('''SELECT * FROM tracks WHERE (playlist_id=? AND youtube_id=?)''',
                    (track.playlist_id_, track.youtube_id_,))
        entry = cur.fetchone()
        if entry is None:
            print("Validation: Adding to playlist", track.get_fields()[1])
            cur.execute(
                '''INSERT INTO tracks(playlist_id,youtube_id,title,file_location,user_added,date_added,num_plays) VALUES(?,?,?,?,?,?,?)''', track.get_fields()[1:])
            rid = cur.lastrowid
            self.connection_.commit()
            self.close()
            return (True, rid)
    ...
    def remove_track_from_playlist(self, playlist_id, track):
        self.open()
        cur = self.connection_.cursor()
        cur.execute('''DELETE FROM tracks WHERE (playlist_id=? AND youtube_id=?)''',
                    (playlist_id, track.youtube_id_))
        self.connection_.commit()
        self.close()
    ...
    def remove_playlist(self, playlist_id):
        self.open()
        cur = self.connection_.cursor()
        cur.execute('''DELETE FROM tracks WHERE playlist_id=?''',
                    (playlist_id,))
    ...
    def add_playlist_request(self, playlist, num_requests=1):
        self.open()
        cur = self.connection_.cursor()
        cur.execute('''INSERT INTO playlist_requests(playlist_id,num_requests) VALUES(?,?)''',
                    (playlist.rid_, num_requests))
        self.connection_.commit()
        self.close()
        return True
    ...
    def pop_playlist_request(self):
        ...
        if rows:
            ...
            for i in range(num_playlists):
                ...
                cur.execute(
                    '''DELETE FROM playlist_requests WHERE id=?''', (playlist_request_id,))
                self.connection_.commit()
                cur.execute(
                    '''SELECT * FROM tracks WHERE playlist_id=?''', (playlist_id,))
